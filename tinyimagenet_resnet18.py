...
parser.add_argument('--data', metavar='DIR',default='./tiny-224',#'/data1/ILSVRC2012',#
...
parser.add_argument('--final_lr', default=0.01, type=float, metavar='U',
                    help='final learning rate of AdaSB')
parser.add_argument('--upper', default=20.0, type=float, metavar='U',
                    help='upper bound of lr in AdaSB')
parser.add_argument('--lower', default=0.05, type=float, metavar='L',
                    help='lower bound of lr in AdaSB')
...
parser.add_argument('--distributed', default=False, dest='distributed', action='store_true',
                    help='use pre-trained model')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.optimizer=='adabs':
        log_dir=f'logs/res/{args.optimizer}-{args.lr}-final_lr{args.final_lr}-decay{args.weight_decay}-seed{args.seed}-lower{args.lower}-upper{args.upper}'
        log_dir += "-epochs" + str(args.epochs)
        
    else:
        log_dir=f'logs/res/{args.optimizer}-{args.lr}-decay{args.weight_decay}-eps{args.eps}-seed{args.seed}'
        log_dir += "-m" + str(args.momentum)
        log_dir += "-epochs" + str(args.epochs)
    args.save = log_dir + '/'
    writer = SummaryWriter(log_dir=log_dir)
    
    title = 'Tiny-Imagenet-224-' + args.arch

    if not os.path.isdir(log_dir):
        mkdir_p(log_dir)
    logger = Logger(os.path.join(log_dir, 'log.txt'), title=title)
    logger.set_names(['Learning Rate', 'Train Loss', 'Valid Loss', 'Train Acc1.', 'Train Acc5.', 'Val Acc1.', 'Val Acc5.',])
    ...
    for layer in model.modules():
      if isinstance(layer, nn.BatchNorm2d):
        layer.float()
    '''
    distributed = False  # not distributed 
    ...
    if args.optimizer == 'sgd' and (not args.centralize):
    ...
    elif args.optimizer == 'adar':
        optimizer = AdaR(model.parameters(), args.lr, eps=args.eps, betas=(args.beta1, args.beta2), weight_decouple = args.weight_decouple,
                              weight_decay = args.weight_decay, fixed_decay = args.fixed_decay)
        ...
        optimizer = optim.Adam(model.parameters(), args.lr, eps=args.eps, betas=(args.beta1, args.beta2), weight_decay = args.weight_decay)
    elif args.optimizer == 'adashift':
        optimizer = AdaShift(model.parameters(), args.lr, eps=args.eps, betas=(args.beta1, args.beta2))
    elif args.optimizer == 'adan':
        optimizer = Adan(model.parameters(), args.lr, eps=args.eps, weight_decay = args.weight_decay)
    elif args.optimizer == 'adaplus':
        optimizer = AdaPlus(model.parameters(), args.lr, eps=args.eps, betas=(args.beta1, args.beta2), weight_decay = args.weight_decay)
    elif args.optimizer == 'adam_win':
        optimizer = Adam_Win(model.parameters(), args.lr, eps=args.eps, betas=(args.beta1, args.beta2), weight_decay = args.weight_decay, acceleration_mode='win')
        ...
        optimizer = Yogi(model.parameters(),lr=args.lr,betas=(args.beta1, args.beta2))
    elif args.optimizer == 'adan':
        optimizer = Adan(model.parameters(),lr=args.lr, betas=(args.beta1,args.beta1, args.beta2))
    elif args.optimizer == 'adam_win':
        optimizer = Adam_Win(model.parameters(), lr=args.lr, weight_decay=2e-5, betas=(args.beta1, args.beta2), acceleration_mode='win')
    elif args.optimizer == 'radam':
        optimizer = RAdam(model.parameters(), lr=args.lr,betas=(args.beta1, args.beta2))
    elif args.optimizer == 'nosadam':
        optimizer = NosAdam(model.parameters(), lr=args.lr,betas=(args.beta1, args.beta2))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1_val, acc5_val, loss_val = validate(val_loader, model, criterion, args)

        train1.append(acc1_train.data.cpu().numpy())
        train5.append(acc5_train.data.cpu().numpy())
        test1.append(acc1_val.data.cpu().numpy())
        test5.append(acc5_val.data.cpu().numpy())
        results = {}
        results['train1'] = acc1_train
        results['train5'] = acc5_train
        results['test1'] = acc1_val
        results['test5'] = acc5_val

        print(args.optimizer)

        logger.append([state['lr'], loss_train, loss_val, acc1_train, acc5_train, acc1_val, acc5_val])
