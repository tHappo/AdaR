...
parser.add_argument('--partial', type=float, default=4,
                    help='PAdam partially adaptive parameter')
...
if args.optimizer=='adadb':
    log_dir=f'logs/results/{args.optimizer}-{args.lr}-nlayers{args.nlayers}-decay{args.wdecay}-seed{args.seed}-eps{args.eps}'
...
title = 'PTB-' + str(args.nlayers) + args.optimizer

if not os.path.isdir(log_dir):
    mkdir_p(log_dir)
logger = Logger(os.path.join(log_dir, 'log.txt'), title=title)
logger.set_names(['Epochs', 'Train Loss', 'Valid Loss', 'Val PPL', 'Val BPC', 'Time'])
...
try:
    ...
    if args.optimizer == 'swats':
        optimizer = SWATS(params, lr=args.lr, weight_decay=args.wdecay)
    if args.optimizer == 'radam':
        optimizer = RAdam(params, lr=args.lr, weight_decay=args.wdecay)
    if args.optimizer == 'adar':
        optimizer = AdaR(params, lr=args.lr, weight_decay=args.wdecay)
    if args.optimizer == 'padam':
        optimizer = PAdam(params, lr=args.lr, partial=1/args.partial, weight_decay=args.wdecay)
    if args.optimizer == 'adamw':
        optimizer = AdamW(params, lr=args.lr, weight_decay=args.wdecay)
    if args.optimizer == 'adabound':
        optimizer = AdaBound(params, lr=args.lr, weight_decay=args.wdecay)
    ...
    if args.optimizer.lower() == 'adadb':
        optimizer = AdaDB(params, lr=args.lr, weight_decay=args.wdecay,
                            eps=args.eps, betas=(args.beta1, args.beta2))
    if args.optimizer.lower() == 'adam_win':
        optimizer = Adam_Win(params, lr=args.lr, weight_decay=args.wdecay,
                             betas=(args.beta1, args.beta2), acceleration_mode='win')
    if args.optimizer.lower() == 'adan':
        optimizer = Adan(params, lr=args.lr, weight_decay=args.wdecay,
                             betas=(args.beta1, args.beta1, args.beta2))
    if args.optimizer.lower() == 'nero':
        optimizer = Nero(params, lr=args.lr)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if 't0' in optimizer.param_groups[0]:
            ...
            logger.append([epoch , train_loss, val_loss2, math.exp(val_loss2), (val_loss2 / math.log(2)), (epoch_end_time - epoch_start_time)])
            ...
            logger.append([epoch, train_loss, val_loss, math.exp(val_loss), (val_loss / math.log(2)), (epoch_end_time - epoch_start_time)])
...
logger.append([000, test_loss, test_loss, math.exp(test_loss), (test_loss / math.log(2)), 000])
